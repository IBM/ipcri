	...
	for (i = 0; i < tot_buff; i++) {
		...
		if (isshake(mode)) {
			if (bits == SHAKE128_DIGEST) {
				...
				for (j = 0; j < 1000; j++) {
					shake128_call_progressive(bits, dummy_msg, in_len, out);
				}
				clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &end_);
			} else if (bits == SHAKE256_DIGEST) {
				clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &start_);
				for (j = 0; j < 1000; j++) {
					shake256_call_progressive(bits, dummy_msg, in_len, out);
				}
				clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &end_);
			}
		} else {
			clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &start_);
			for (j = 0; j < 1000; j++) {
				sha3_call(bits, dummy_msg, in_len, out);
			}
			clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &end_);
		}
