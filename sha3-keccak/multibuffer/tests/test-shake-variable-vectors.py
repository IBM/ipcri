...
def set_shared_lib() -> ctypes.CDLL:
    ...
    lib.shake128_call_progressive_mult.argtypes = (ctypes.c_uint32, ctypes.c_char_p, ctypes.c_char_p, ctypes.c_int, ctypes.c_int, ctypes.POINTER(ctypes.c_ubyte), ctypes.POINTER(ctypes.c_ubyte))
    lib.shake128_call_progressive_mult.restype = None

    lib.shake256_call_progressive_mult.argtypes = (ctypes.c_uint32, ctypes.c_char_p, ctypes.c_char_p, ctypes.c_int, ctypes.c_int, ctypes.POINTER(ctypes.c_ubyte), ctypes.POINTER(ctypes.c_ubyte))
    lib.shake256_call_progressive_mult.restype = None
    return lib

def shake128_hash(lib, msg: bytes, output_len: int) -> bytes:
    output = (ctypes.c_ubyte * output_len)()
    output2 = (ctypes.c_ubyte * output_len)()
    msg_len = len(msg)
    lib.shake128_call_progressive_mult((output_len), msg, msg, msg_len, msg_len, output, output2)
    return bytes(output)
...
def shake256_hash(lib, msg: bytes, output_len: int) -> bytes:
    ...
    output2 = (ctypes.c_ubyte * output_len)()
    msg_len = len(msg)
    lib.shake256_call_progressive_mult((output_len), msg, msg, msg_len, msg_len, output, output2)
    return bytes(output)

def output(count: int, msg: str, output_hash: bytes, expected_output: str):
    out_hex = base64.b16encode(output_hash).decode('utf-8').lower()
    expected_output_len = len(expected_output) // 2 
    if out_hex == expected_output:
...
def main():
    ...
    parser.add_argument('mode', type=str, help="Enter a calltion name to apply the output test 'e.g. shake-256, shake-128..' : ")
