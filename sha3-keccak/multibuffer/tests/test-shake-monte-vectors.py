...
#
# helper src: https://csrc.nist.gov/CSRC/media/Projects/Cryptographic-Algorithm-Validation-Program/documents/sha3/sha3vs.pdf#page=26

import ctypes

lib = ctypes.CDLL('../libkeccak.so')
lib.shake128_call_mult.restype = None
lib.shake128_call_mult.argtypes = (ctypes.c_uint32, ctypes.c_char_p, ctypes.c_char_p, ctypes.c_int, ctypes.c_int, ctypes.POINTER(ctypes.c_ubyte), ctypes.POINTER(ctypes.c_ubyte))
...
def shake128(msg: bytes, output_len: int) -> bytes:
    ...
    msg_len = len(msg)
    lib.shake128_call_mult((output_len*8)//2, msg, msg, msg_len, msg_len, output, output2)
    return bytes(output)
