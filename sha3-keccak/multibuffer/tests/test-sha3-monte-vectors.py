...
def set_shared_lib() -> ctypes.CDLL:
    ...
    lib.sha3_call_mult.argtypes = (ctypes.c_uint32, ctypes.c_char_p, ctypes.c_char_p, ctypes.c_int, ctypes.c_int, ctypes.POINTER(ctypes.c_ubyte), ctypes.POINTER(ctypes.c_ubyte))
    return lib
...
def sha(lib, bits, message) -> bytes:
    ...
    md2 = (ctypes.c_uint8 * (tobytes(bits)))()
    lib.sha3_call_mult(bits, message, message, msgsz, msgsz, md, md2)
    return bytes(md)  
